...
//        double lamda = 1e-4;
        ...
        for (int t = 1; t <= 10; ++t) {
            ...
            for (int i = 0; i < iteration; ++i) {
                ...
                double wx = innerProduct(w, randomTrainingData);
                for (int j = 0; j < w.size(); ++j) {
                    w.set(j, (1 - 1.0 / (i + 1)) * w.get(j) + 1.0 / (lamda * (i + 1)) * randomLabel / (1 + Math.exp(randomLabel * wx)) * randomTrainingData.get(j));
                }
            }
